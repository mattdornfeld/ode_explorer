...
class MainWindow(QMainWindow):
  ...
  def init_ui(self):
    ...
    self.add_text_box(update_fun=self.on_nm_change,
      ...
      row=3, col=2)
    self.add_text_box(update_fun=self.on_zeta_change,
      name='zeta', default_value=self.zeta, unit='', conversion=1, 
      row=4, col=2)
  ...
  def set_defaults(self):
     ...
     # self.q0 = np.zeros(2*self.nm+1)
      ...
      self.params = cp.calc_parameters(self.p, self.r_folds, self.r_folds, 
        self.s.imag, self.s.real, self.nm, self.zeta)
  ...
  def on_r_back_change(self, text):
      if len(text) != 0:
        ...
        unit = self.controls_dict[name][4]
        conversion = self.controls_dict[name][5]
        label_text = (name + ' (' + str(self.r_back * conversion) + 
        ' ' + unit + ')')
        self.controls_dict[name][1].setText(label_text)
        self.pbar.setValue(0)

  def on_zeta_change(self, text):
      if len(text) != 0:
        try:
          self.zeta = float(text)
        except:
          raise ValueError('Input must be a float')
        ...
        unit = self.controls_dict[name][4]
        conversion = self.controls_dict[name][5]
        label_text = (name + ' (' + str(self.zeta * conversion) + 
        ' ' + unit + ')')
        self.controls_dict[name][1].setText(label_text)
        self.pbar.setValue(0)
